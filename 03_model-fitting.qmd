...
Y_i = \beta_0 + U_i,
$$ {#eq-std-lm-galicia} where the $U_i$ are independent Gaussian random variables with mean zero and variance $\omega^2$. Our goal at this stage is to assess whether this assumption of independence is supported by the data, or whether there is evidence of spatial correlation.
...
In this section, we focus on spatially referenced outcomes $Y_i$ that are continuous. We first consider the simpler case of a single measurement $Y_i$ per location $x_i$. Recalling the class of generalized linear models introduced in @sec-geostat-models, the linear predictor takes the form $$
...
By examining the results shown in panel (a) of @fig-galicia-matern, we observe that, as expected, smaller values for $\kappa$ lead to smaller point estimates for $\omega^2$ and viceversa. This begs the question, what should be our chosen value for $\kappa$?
...
\begin{aligned}
Y_{ij} = \beta_0 &+ \overbrace{\beta_1 d(x_i) + S(x_i)}^\text{Location-level effect} + \underbrace{\sum_{h=1}^{n_{V}} I(x_i \in \mathcal{A}_h) \times V_h}_\text{Region effect} \\
&+ \overbrace{\sum_{k=1}^{n_{Z}} I(x_i \in \mathcal{B}_k) \times Z_k}^\text{Province effect} + \underbrace{U_{ij}}_\text{Measurement error}
\end{aligned}
...
In section @sec-agg-count-exp, we carried out the exploratory analysis of the river-blindness prevalence data to assess how elevation could be introduced into the model. In @sec-empirical-variog-bin, we then tested for residual spatial correlation using the empirical variogram and log-transformed elevation as a covariate. Based on these results, we can now formulate a Binomial geostatistical model which we then fit to the data. Hence, we extend the model presented in @eq-liberia-bin-mixed-elev through the inclusion of a spatial Gaussian process, $S(x)$, and write $$
...
Using the `dist_summaries` function, we can see that, among the locations of the data-set, the average distance is about 55km and the minimum is about 387 meters. The estimate of phi of 4km is thus relatively small compared to the distances between locations in the data. In @sec-geo-prediction, we will further investigate to what extent this small-scale spatial correlation can affect the predictions of the An. Gambiae counts.
...
In geostatistical models, the concept of residuals, which are widely used in regression modelling to carry out model diagnostics, becomes more nuanced than in standard regression models (i.e., models that do not include random effects) . Generally, residuals aim to quantify the discrepancy between observed outcomes and what the model predicts. However, in GLGMs, there are multiple valid definitions of residuals.
...
Let $W_i$ denote the sum of all the random effects in the model, whether $W_i = S(x_i)$, $W_i = S(x_i) + Z_i$, or any other random effects structure. In addition to RERs, other types of residuals more common in standard regression modeling can also be used, however, because of the presence of random effects $W_i$, their definition becomes more convoluted.

Let us consider the *Pearson residuals*. Conditionally on $W_i$ we can define these as: $$
{\rm PR}_i(W_i) = \frac{y_i - \hat{\text{E}}\left[Y_i]}{\sqrt{\hat{\text{Var}}\left[Y_i]}}
$$

where $\hat{\text{E}}\left[Y_i\right]$ and $\hat{\text{Var}}\left[Y_i \right]$ are estimates of the predictive expectation and variance of $Y_i$. Using Monte Carlo samples, say $W_{i,(j)}$ for $j = 1, \ldots, B$, from the distribution of $W_i$ given the data (see @sec-mcmc-mala for how these samples are generated in `RiskMap`), the expectations can be estimated as:

$$
\hat{\text{E}}\left[Y_i \right] = \frac{1}{B} \sum_{j=1}^B \hat{\text{E}}\left[Y_i \mid W_{i,(j)}\right], \quad
\hat{\text{Var}}\left[Y_i \right] = \frac{1}{B} \sum_{j=1}^B \hat{\text{Var}}\left[Y_i \mid W_{i,(j)}\right]
$$
...
In maximum likelihood estimation, we aim to maximize the likelihood function to find the parameters that make our data the most likely to have been observed. Monte Carlo Maximum Likelihood (MCML) (see the seminal work by @geyer1991) is designed to handle situations where the likelihood function is intractable. In the case of a non-Gaussian generalized linear geostatistical model, its likelihood function involves a high-dimensional integral which can be approximated using MCML.

One way to apply Monte Carlo methods in this context is through *importance sampling*. Importance sampling allows us to approximate the likelihood function by rewriting the likelihood in a form that can be estimated using samples from an easier distribution, known as the *importance sampling distribution*. To this end, we then rewrite @eq-glgm-lik2 as 
$$
\begin{aligned}
L(\theta) 
&= \int [W, Y; \theta] \: dW  \\
...
\end{aligned} 
$$ {#eq-mcml1} 
where $\theta_0$ represents our best guess of $\theta_0$ and the expectation in the equation above is taken with respect to $[W | Y; \theta_0]$, which represent our *importance sampling distribution*.
...
The Metropolis-Hastings algorithm} is a foundational technique in the field of MCMC methods, which are widely used for sampling from complex probability distributions. The primary goal of MCMC methods is to generate a sequence of samples from a target distribution where direct sampling is challenging, such as the distribution of $[W | Y]$. The Metropolis-Hastings algorithm, first introduced by @metropolis1953equation and later generalized by @hastings1970monte, achieves this by constructing a Markov chain that has $[W \| Y]$ as its stationary distribution. In what follows, we shall use $\pi(W)$ to denote the density function of $[W | Y]$ up to a constant of proportionality.
...
The functions of the `RiskMap` package for fitting Binomial and Poisson geostatistical models implement the *Metropolis-Adjusted Langevin Algorithm (MALA)*, which is an improvement over traditional Metropolis-Hastings MCMC methods. Introduced by @rossky1978brownian and further developed by @roberts1996exponential, MALA incorporates gradient information from the target distribution to propose more informed moves, potentially leading to faster convergence and more efficient exploration of the target distribution.
...
7.  In Section @sec-glm-parest, we explained that for a linear model with one observation per location, we cannot simultaneously estimate both the measurement error variance $\omega^2$ and the nugget effect variance $\tau^2$. However, if $\omega^2$ is known, we can fix this value and estimate the nugget effect. Suppose that for the Galicia data, the measurement device has a known standard error of $\omega = 0.15$ on the log-scale (i.e., $\omega^2 = 0.0225$). Fit the model $$Y_i = \mu + S(x_i) + Z_i + U_i$$ using `glgpm` by setting `nugget=NULL` and `fix_var_me = 0.0225` in the `gp()` function, with $\kappa = 1.5$. Compare the estimates of $\sigma^2$ and $\phi$ with those obtained from the model without the nugget effect (Equation 3.16). What is the estimated nugget variance $\hat{\tau}^2$? Repeat this analysis for different values of fixed measurement error variance: $\omega^2 \in \{0.01, 0.05, 0.10\}$. How do the estimates of $\sigma^2$, $\phi$, and $\tau^2$ change as the assumed measurement error variance increases?

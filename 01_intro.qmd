...
What distinguishes this dataset from the other examples is that it contains both individual- and household-level information. The primary outcome is the result of a rapid diagnostic test for malaria. In this book, we show how to incorporate the hierarchical structure of the data together with the binary nature of the outcome at each stage of the geostatistical analysis.
...
Once estimation has been carried out, the next step is model validation, where diagnostic tools are used to assess whether the fitted model can be trusted to represent the observed variation in the outcome. If the diagnostics indicate a poor fit, the process returns to the formulation stage in order to address the shortcomings revealed by the validation. If no evidence against the model is found, we move on to spatial prediction. At this stage, defining appropriate predictive targets is crucial, as they ensure that the analysis provides meaningful answers to the original research questions and supports decision making. An equally important step is the evaluation of predictive performance. In geostatistical applications, this is particularly challenging because of the spatially sparse nature of the data. Cross-validation offers a practical approach, but its design and interpretation depend on the ultimate goal of the analysis—for example, whether the aim is to draw predictive inferences within the study region or to extend inference beyond the sampled area. In addition, simulation studies provide a powerful complementary tool, allowing us to assess how a model is expected to perform "on average," with the expectation that the results from a given analysis will not deviate substantially from this average behaviour.
